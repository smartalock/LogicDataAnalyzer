...
void LogicDataAnalyzerResults::GenerateBubbleText( U64 frame_index, Channel& channel, DisplayBase display_base )
{
	...
	if (frame.mType == 2) {
		...
		if ((frame.mData1 & 0xFFFF0000) == 0x40600000) {
			uint8_t value = (frame.mData1 & 0x1FE) >> 1;
			char buf[20];
			snprintf(buf, sizeof(buf), " (%d cm)", rev_byte[value]);

			strcat(number_str, buf);
		}
